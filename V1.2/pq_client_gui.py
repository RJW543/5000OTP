...
def decap_shared_secret(ciphertext: bytes, secret_key: bytes) -> bytes:
    ...
    Tries import_secret_key() first, falls back for older oqs bindings.
    """
    try:
        with oqs.KeyEncapsulation(KEM_ALG) as kem:
...
class KyberAESChatClient:
    def __init__(self, master):
        ...
        # IDs and crypto
        self.user_id = None
        # For each peer_id store a symmetric AES key
        self.aes_keys = {}       # peer_id -> aes_key(bytes)
        # For handshakes initiate: peer_id -> Kyber secret key
        self.pending_kem = {}    # peer_id -> secret_key(bytes)

        # Key status UI
        self.key_status_var = tk.StringVar(value="No AES key established.")
        self.key_status_label = None  # created in _build_gui()
    ...
    def _build_gui(self):
        ...
        self.connection_status = ttk.Label(
            user_frame, text="Not connected.", foreground="red"
        )
        ...
        self.message_entry.grid(row=3, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
    ...
    def start_key_exchange(self):
        ...
        try:
            ...
            self.key_status_var.set(f"Key exchange in progress with {peer_id}...")
            self.key_status_label.config(foreground="orange")
        except Exception as e:
    ...
    def send_message(self):
        ...
        try:
            self._append_chat_line(
                f"[ENCRYPT] Ciphertext to {peer_id} (Base64): {encoded_cipher}"
            )
    ...
    def _handle_kem_init(self, sender_id: str, pk_b64: str):
        ...
        try:
            ...
            self._set_key_status_established(sender_id, "responder")
        except Exception as e:
            self._append_chat_line(
    ...
    def _handle_kem_resp(self, sender_id: str, ct_b64: str):
        ...
        try:
            ...
            self._set_key_status_established(sender_id, "initiator")
        except Exception as e:
            self._append_chat_line(
    ...
    def _handle_encrypted_msg(self, sender_id: str, encoded_cipher: str):
        ...
        self._append_chat_line(
            f"[ENCRYPT] Received ciphertext from {sender_id} (Base64): {encoded_cipher}"
        )
    ...
    def _set_key_status_none(self):
        self.key_status_var.set("No AES key established.")
        if self.key_status_label is not None:
            self.key_status_label.config(foreground="red")

    def _set_key_status_established(self, peer_id: str, role: str):
        """
        Visual indicator when key exchange has completed successfully.
        """
        self.key_status_var.set(f"AES key with {peer_id} established ({role}).")
        if self.key_status_label is not None:
