 ...
 * Source: Ekdahl, Johansson, Maximov, Yang — "A new SNOW stream cipher called SNOW-V"
 *         IACR Transactions on Symmetric Cryptology, 2019, Issue 3, pp. 1-42.
 *         Appendix D, reproduced verbatim from the published paper.
 *
 * This implementation is portable C/C++ and is the correct target for
 * ARM (AArch64) devices. The SIMD variant in Appendix E of the same paper
 * uses Intel-specific intrinsics (<immintrin.h>) and is NOT ARM-compatible.
 *
 * Compilation on Raspberry Pi 5 (Cortex-A76):
 *   g++ -O2 -march=armv8.2-a+crypto -o snowv_test main_kat.cpp
 *
 * Compilation on Raspberry Pi Zero 2W (Cortex-A53):
 *   g++ -O2 -march=armv8-a+crypto -o snowv_test main_kat.cpp
 */
...
static u8  Sigma[16]   = {0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15};
static u32 AesKey1[4]  = { 0, 0, 0, 0 };
static u32 AesKey2[4]  = { 0, 0, 0, 0 };
...
#define MKSTEP(j) \
        ...
        MKSTEP(0); MKSTEP(1); MKSTEP(2); MKSTEP(3);
...
#undef MKSTEP
    ...
    {
        if (v & 0x8000) return (v << 1) ^ c;
        else            return (v << 1);
    }

    u16 mul_x_inv(u16 v, u16 d)
    {
        if (v & 0x0001) return (v >> 1) ^ d;
        else            return (v >> 1);
    }
